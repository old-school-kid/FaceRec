    ...
    if len(input_vid) < 24:
        ...
        if pred[0][0]> .95:
            ...
            for (top, right, bottom, left), name in zip(face_locations, face_names):
                ...
                if unlock:
                    cv2.putText(frame, 'Marked present', (frame.shape[1]//2, frame.shape[0]//2), font, 1.0, (255, 255, 255), 1)
                    if flag_win:
                        cv2.imshow('Video', frame)
                    date= str(datetime.datetime.now())[:(str(datetime.datetime.now()).index(' '))+1]
                    time_now= str(datetime.datetime.now())[(str(datetime.datetime.now()).index(' '))+1:]
                    time_now= time_now[:8]
                    date= convert(date)
                    if cond:
                    ...
                    cv2.putText(frame, 'Can not recognise', (frame.shape[1]//2, frame.shape[0]//2), font, 1.0, (255, 255, 255), 1)
        else:
            cv2.putText(frame, 'INVALID', (frame.shape[1]//2, frame.shape[0]//2), font, 1.0, (255, 255, 255), 1)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        #Or hit space on the keyboard to quit!
